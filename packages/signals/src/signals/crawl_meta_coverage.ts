...
export const crawlMetaCoverage: Signal = {
  ...
  async analyze(ctx: AuditContext): Promise<SignalResult> {
    ...
    return {
      ...
      details: {
        total,
        missing: { count: missing.length, ratio: Math.round(missingRatio * 100) },
        tooShort: { count: tooShort.length },
        tooLong: { count: tooLong.length },
        duplicated: { groups: duplicateGroups.length, urls: duplicatedUrls },
        h1: { missing: missingH1.length, multiple: multipleH1.length },
        ok,
      },
