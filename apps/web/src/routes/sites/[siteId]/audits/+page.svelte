<script lang="ts">
  ...
  function statusBadge(status: string) {
    switch (status) {
      case 'completed': return 'bg-green-50 dark:bg-green-950/30 text-green-700 dark:text-green-400'
      case 'running': return 'bg-blue-50 dark:bg-blue-950/30 text-blue-700 dark:text-blue-400'
      case 'pending': return 'bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-400'
      case 'failed': return 'bg-red-50 dark:bg-red-950/30 text-red-700 dark:text-red-400'
      default: return 'bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400'
    }
  }
...
{:else if site}
  <div class="mb-8">
    <a href="/sites/{siteId}" class="text-sm text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200">← {site.name}</a>
    <h1 class="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-1">Historique des audits</h1>
  </div>

  <!-- Tendance globale -->
  {#if sparkScores.length >= 2}
    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 mb-6 flex items-center gap-6">
      <div>
        <p class="text-xs text-slate-500 dark:text-slate-400 mb-1">Tendance — score global</p>
        <Sparkline scores={sparkScores} width={200} height={40} />
      </div>
      <div class="text-center">
        <p class="text-xs text-slate-400 dark:text-slate-500">Dernier</p>
      ...
      {#if sparkScores.length >= 2}
        ...
        <div class="text-center">
          <p class="text-xs text-slate-400 dark:text-slate-500">Évolution</p>
      ...
      {/if}
      <div class="text-center">
        <p class="text-xs text-slate-400 dark:text-slate-500">Audits</p>
        <p class="text-lg font-bold text-slate-700 dark:text-slate-300">{completedAudits.length}</p>
      </div>
    </div>
  {/if}
  ...
  <!-- Liste des audits -->
  {#if history.length === 0}
    <div class="text-center py-12 text-slate-500 dark:text-slate-400">Aucun audit effectué.</div>
  {:else}
      ...
      <div class="divide-y divide-slate-100 dark:divide-slate-800">
        {#each history as audit}
          <a
            ...
            class="flex items-center justify-between px-5 py-3 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors {audit.status !== 'completed' ? 'cursor-default' : ''}">
            <div class="flex items-center gap-3">
              <span class="text-xs px-2 py-0.5 rounded-full font-medium {statusBadge(audit.status)}">
                {statusLabel(audit.status)}
              </span>
              <div>
                <p class="text-sm text-slate-700 dark:text-slate-300">{formatDate(audit.createdAt)}</p>
                {#if audit.completedAt && audit.startedAt}
                  {@const durationMs = new Date(audit.completedAt).getTime() - new Date(audit.startedAt).getTime()}
                  <p class="text-xs text-slate-400 dark:text-slate-500">{(durationMs / 1000).toFixed(1)}s</p>
                {/if}
            ...
            {#if audit.scores}
              <div class="flex items-center gap-4">
                ...
                {/each}
                <div class="text-center pl-3 border-l border-slate-200 dark:border-slate-700">
